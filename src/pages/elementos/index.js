...
const mockElements = [
  {
    ...
    element_quantity: "250g",
    element_validity: new Date(2026, 2, 26).toISOString(),
    element_admin_level: null,
    element_image:
      "https://images.unsplash.com/photo-1596541530347-130282163b7e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    lab_id: labIdMock,
    element_molar_mass: "58.44 g/mol",
    element_physical_state: "Sólido",
  },
  {
    id: "2",
    element_name: "Éter dietílico",
    element_cas_number: "7312-12-6",
    element_ec_number: "231-598-3",
    element_quantity: "390g",
    element_validity: new Date(2029, 3, 12).toISOString(),
    element_admin_level: "Elemento sob cuidados do exército Brasileiro",
    element_image:
      "https://images.unsplash.com/photo-1579201948496-c68bb45e54d3?q=80&w=1935&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    lab_id: labIdMock,
    element_molar_mass: "74.12 g/mol",
    element_physical_state: "Líquido",
  },
  {
    id: "3",
    element_name: "Benzeno hidratado",
    element_cas_number: "3165-32-3",
    element_ec_number: "941-538-1",
    element_quantity: "500g",
    element_validity: new Date(2030, 1, 13).toISOString(),
    element_admin_level: "Elemento sob cuidados da polícia federal",
    element_image:
      "https://images.unsplash.com/photo-1579201948496-c68bb45e54d3?q=80&w=1935&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    lab_id: labIdMock,
    element_molar_mass: "78.11 g/mol",
    element_physical_state: "Líquido",
  },
  {
    id: "4",
    element_name: "Cloreto salinico",
    element_cas_number: "7984-19-5",
    element_ec_number: "654-111-3",
    element_quantity: "194g",
    element_validity: new Date(2056, 7, 4).toISOString(),
    element_admin_level: null,
    element_image:
      "https://images.unsplash.com/photo-1596541530347-130282163b7e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    lab_id: labIdMock,
    element_molar_mass: "110.12 g/mol",
    element_physical_state: "Gás",
  },
];
...
const ElementListItem = ({ element, onPress }) => {
  const formattedDate = formatDisplayField(
    element.element_validity,
    "element_validity"
  );
...
function InventoryScreen() {
  // Pegando as informações do bottomSheet
  ...
  const fetchElements = useCallback(async () => {
    ...
    try {
      ...
      const filteredMockElements = mockElements.filter(
        (e) => e.lab_id === labId
      );
      ...
      if (response.status && response.data) {
        ...
        Alert.alert(
          "Erro",
          response.msg || "Não foi possível carregar o inventário."
        );
      }
    } catch (error) {
...
function ElementInfoScreen() {
  ...
  const fetchElementInfo = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Erro de Conexão",
        "Falha ao carregar informações do elemento."
      );
    }
    setIsLoading(false);
  }, [elementId]);
  ...
  const handleDeleteElement = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        Alert.alert(
          "Erro",
          response.msg || "Não foi possível excluir o elemento."
        );
      }
    } catch (error) {
  ...
  const EditableField = ({ field, label, value, isTitle = false }) => {
    ...
    return (
      <FieldContainer
        ...
        onPress={
          isEditing ? () => openEditModal(field, elementInfo[field]) : null
        }
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.infoScrollContainer}>
        ...
        <EditableField
          ...
          value={elementInfo.element_validity}
        />
        <EditableField
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <Text style={styles.modalTitle}>
              Editar{" "}
              {editField === "element_name" ? "nome do elemento" : editField}
            </Text>
            <TextInput
      ...
      >
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            >
              Esta é uma ação irreversível e não pode ser desfeita.
            </Text>
            <View style={styles.modalButtonContainer}>
              <TouchableOpacity
                style={styles.modalCancelButton}
      ...
      <Modal animationType="fade" transparent={true} visible={qrModalVisible}>
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            >
              <Text style={{ fontSize: 10, color: colors.primary_text_gray }}>
                [QR CODE SIMULADO]
              </Text>
              <Text style={{ fontSize: 8, color: colors.primary_text_gray }}>
                ID: {elementId}
              </Text>
            </View>
            <TouchableOpacity
  ...
  async function handleRegisterElement() {
    ...
    try {
      ...
      if (response.status) {
        ...
        Alert.alert(
          "Erro",
          response.msg || "Não foi possível registrar o novo elemento."
        );
      }
    } catch (error) {
