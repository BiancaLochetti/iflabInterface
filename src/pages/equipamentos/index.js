...
import { View, Text, TouchableOpacity, ScrollView, TextInput, Image, Alert, Modal, ActivityIndicator } from "react-native";
...
const mockEquipments = [
    ...
    { id: "e1", equipment_name: "Balança Analítica", equipment_quantity: "2", equipment_quality: "Excelente", equipment_description: "A balança analítica é um equipamento de alta precisão...", equipment_admin_level: "IFLab", equipment_image: 'https://via.placeholder.com/600/CCCCCC/808080?text=Balança+Analítica', lab_id: labIdMock }, 
    { id: "e2", equipment_name: "Bico de Bunsen", equipment_quantity: "1", equipment_quality: "Boa", equipment_description: "Aquecimento e esterilização", equipment_admin_level: "Elemento sob cuidados do exército Brasileiro", equipment_image: 'https://via.placeholder.com/600/CCCCCC/808080?text=Bico+de+Bunsen', lab_id: labIdMock }, 
    { id: "e3", equipment_name: "Bureta", equipment_quantity: "4", equipment_quality: "Regular", equipment_description: "Titulações para adicionar líquido com pressão", equipment_admin_level: "Elemento sob cuidados da polícia federal", equipment_image: 'https://via.placeholder.com/600/CCCCCC/808080?text=Bureta', lab_id: labIdMock }, 
    { id: "e4", equipment_name: "Pipeta", equipment_quantity: "8", equipment_quality: "Péssima", equipment_description: "Utilizada em medição em alta precisão", equipment_admin_level: "IFLab", equipment_image: 'https://via.placeholder.com/600/CCCCCC/808080?text=Pipeta', lab_id: labIdMock }, 
];
...
const EquipmentListItem = ({ equipment, onPress }) => {
    return (
        <TouchableOpacity style={styles.listItemContainer} onPress={() => onPress(equipment.id)}>
            <View style={styles.listItemTextContent}>
                <Text style={styles.listItemName}>Nome: {equipment.equipment_name}</Text>
                <Text style={styles.listItemDetail} numberOfLines={1}>Descrição: {equipment.equipment_description}</Text>
                {equipment.equipment_admin_level && equipment.equipment_admin_level !== 'IFLab' && ( 
                    <View style={styles.listItemAdminLevelContainer}>
                        <AntDesign name="infocirlceo" size={14} color={colors.primary_text_gray} /> 
            ...
            <View style={styles.listItemRightContent}>
                <Text style={styles.listItemQuantity}>{equipment.equipment_quantity} uni</Text> 
            </View>
        </TouchableOpacity>
    );
};
...
function EquipmentInventoryScreen() {
    ...
    const fetchEquipments = useCallback(async () => {
        ...
        try {
            ...
            const filteredMockEquipments = mockEquipments.filter(e => e.lab_id === labId);
            const response = { status: true, data: filteredMockEquipments };
            
            if (response.status && response.data) {
                setEquipments(response.data);
            } else {
                Alert.alert("Erro", response.msg || "Não foi possível carregar o inventário de equipamentos.");
            }
        } catch (error) {
    ...
    const filteredEquipments = equipments.filter(equipment =>
        ...
        equipment.equipment_description.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    if (isLoading && equipments.length === 0) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={{ marginTop: 10, color: colors.primary_text_gray }}>Carregando Inventário de Equipamentos...</Text>
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.searchContainer}>
                ...
                />
                <TouchableOpacity onPress={handleAddEquipmentPress} style={styles.addElementButton}>
            ...
            <ScrollView contentContainerStyle={styles.listContainer}>
                {filteredEquipments.length > 0 ? (
                    ...
                    <Text style={styles.noElementsText}>Nenhum equipamento encontrado neste laboratório.</Text>
                )}
            </ScrollView>
        </View>
    );
...
function EquipmentInfoScreen({ route }) { 
    ...
    const fetchEquipmentInfo = useCallback(async () => {
        ...
        try {
            ...
            Alert.alert("Erro de Conexão", "Falha ao carregar informações do equipamento.");
        }
        setIsLoading(false);
    }, [equipmentId]);
    ...
    useEffect(() => { 
        if (!equipmentId) {
            ...
            Alert.alert("Erro de Roteamento", "ID do equipamento não fornecido. Voltando.");
            navigation.goBack();
            return;
        }
    ...
    const handleDeleteEquipment = async () => { 
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 500));
            const response = { status: true, msg: "Equipamento excluído com sucesso." };
            
            if (response.status) {
                Alert.alert("Sucesso", response.msg);
                navigation.goBack();
            } else {
                 Alert.alert("Erro", response.msg || "Não foi possível excluir o equipamento.");
            }
        } catch (error) {
        ...
        if (field === 'equipment_quality' && !isTitle) {
             return (
                <FieldContainer 
                    ...
                    onPress={isEditing ? () => openEditModal(field, equipmentInfo[field]) : null}
    ...
    if (!equipmentId || isLoading || !equipmentInfo) {
         return (
            <View style={styles.loadingContainer}>
                ...
                {equipmentId && <ActivityIndicator size="large" color={colors.secundary_green} />}
                <Text style={{ marginTop: 10, color: colors.primary_text_gray }}>
                    {equipmentId ? "Carregando Equipamento..." : "Erro. A tela deve voltar automaticamente."}
                </Text>
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.infoScrollContainer}>
                ...
                <EditableField field='equipment_name' label="" value={equipmentInfo.equipment_name} isTitle={true} />

                <View style={styles.infoGrid}>
                    <View style={styles.infoGridItemContainer}>
                        <EditableField field='equipment_quantity' label="Quantidade:" value={equipmentInfo.equipment_quantity} />
                    </View>
                    <View style={styles.infoGridItemContainer}>
                        <EditableField field='equipment_admin_level' label="Admin:" value={equipmentInfo.equipment_admin_level || "IFLab"} /> 
                    </View>
                </View>
                
                {/* Qualidade - renderizado com estrelas */}
                <EditableField field='equipment_quality' label="Qualidade:" value={equipmentInfo.equipment_quality} /> 
                
                {/* Descrição em campo próprio */}
                <EditableField field='equipment_description' label="Descrição:" value={equipmentInfo.equipment_description} />
...
function NewEquipmentScreen() { /* ... */
    ...
    async function handleRegisterEquipment() { /* ... */
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 500));
            const response = { status: true, msg: "Equipamento registrado com sucesso." };

            if (response.status) {
                Alert.alert("Sucesso", response.msg);
                navigation.goBack(); 
            } else {
                Alert.alert("Erro", response.msg || "Não foi possível registrar o novo equipamento.");
            }
        } catch (error) {
    ...
    if (isLoading) {
         return (
            <View style={styles.loadingContainer}>
                <ActivityIndicator size="large" color={colors.secundary_green} />
                <Text style={{ marginTop: 10, color: colors.primary_text_gray }}>Registrando Equipamento...</Text>
            </View>
        );
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.newElementScrollContainer}>
                ...
                <TextInput style={styles.newElementInput} placeholder="* Nome do equipamento" value={name} onChangeText={setName} placeholderTextColor={colors.input_text_gray} />
                <TextInput style={styles.newElementInput} placeholder="* Quantidade (ex: 5)" value={quantity} onChangeText={setQuantity} keyboardType="numeric" placeholderTextColor={colors.input_text_gray} />
                <TextInput style={styles.newElementInput} placeholder="* Qualidade (ex: Excelente, Boa, Regular)" value={quality} onChangeText={setQuality} placeholderTextColor={colors.input_text_gray} />
                <TextInput style={[styles.newElementInput, {height: 80}]} placeholder="Descrição (Opcional)" value={description} onChangeText={setDescription} multiline={true} placeholderTextColor={colors.input_text_gray} />
...
export default function EquipmentsStack() {
    return (
        <Stack.Navigator screenOptions={{ headerShown: false }}>
            ...
            <Stack.Screen name="NewEquipment" component={NewEquipmentScreen} options={{ presentation: 'modal' }} />
        </Stack.Navigator>
    );
}